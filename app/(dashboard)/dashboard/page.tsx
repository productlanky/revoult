  ...
  useEffect(() => {
    async function fetchLiveRates() {
      try {
        ...
        const res = await fetch("/api/rates?from=USD&to=EUR,GBP,JPY,CAD,AUD");

        if (!res.ok) throw new Error("Server returned an operational error code");
            ...
            {virtualCard && (
              <div className="grid grid-cols-2 gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={isFreezing}
                  className={`py-3 rounded-[16px] border flex items-center justify-center gap-2 text-sm font-bold transition-colors ${virtualCard.status === 'Frozen'
...
function HeroAction({ icon: Icon, label, highlight = false, link }: { icon: any, label: string, highlight?: boolean, link: string }) {
  return (
    <Link href={link} className="flex flex-col items-center justify-center gap-2 group">
      <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-[20px] flex items-center justify-center transition-all duration-300 ${highlight
