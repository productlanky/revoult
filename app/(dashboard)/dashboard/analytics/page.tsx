...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeTx = onSnapshot(
      ...
      (error) => {
        if (error.code === "permission-denied") {
          console.log("Analytics transactions stream safely detached.");
        } else {
          console.error("Firestore analytics tx error:", error);
        }
        setLoading(false);
      }
    );
    ...
    const unsubscribeBudgets = onSnapshot(
      ...
      (error) => {
        if (error.code === "permission-denied") {
          console.log("Analytics budgets stream safely detached.");
        } else {
          console.error("Firestore analytics budgets error:", error);
        }
      }
    );
                  ...
                  {monthlySpendData.map((data, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`w-full max-w-[40px] rounded-t-xl transition-all duration-500 ease-out ${isActive && ytdSpent > 0
